...
from DrawHistogram import plotSysComparison
#from runTUnfold import drawUncertainties
...
def makePlots( name, ext='png', outputDir='Plots/' ):
        ...
        if name.endswith( ivar.split('_')[0] ):
            ...
            if args.process.startswith('MCClosure'):
                ...
                plotSysComparison( dictHistos[ivar]['reco'].Clone(),
                                    dictUncHistos,
                                    ivar+'_'+args.selection+'_'+outputLabel+'_allSys',
                                    labelX=nSubVariables[ivar]['label'],
    ...
    if not args.only:
        combinePlots( name+'_'+args.selection, dictHistos, numBinsList, mainHistoLabel, otherHisto, otherHistoLabel, outputLabel, outputDir=outputDir, axisX='' , ratioOnly=( False if args.process.startswith('data') else True ))
        if ratioDict:
...
def drawUncertainties( ivar, unfoldHistoTotUnc, uncerUnfoldHisto, labelX, tlegendAlignment, outputName ):
    ...
    if args.ext.startswith('pdf'):
        canUnc.SaveAs( outputName.replace('pdf', 'png') )


##################################################
def combinePlots( name, dictHistos, numBins, mainHistoLabel, otherHisto, otherHistoLabel, outputLabel, axisX='', outputDir='Plots/', ratioOnly=False):
...
def combineRatioPlots( name, ratioDicts, numBins, outputLabel, axisX='', outputDir='Plots/'):
    ...
    legend.AddEntry( dictHistos[ 'combRecoRatio' ], 'reco-level', 'lep' )
    ...
    legend.AddEntry( dictHistos[ 'combGenRatio' ], 'gen-level', 'lep' )
...
if __name__ == '__main__':
    ...
    outputDir = args.outputFolder+'Plots/'+args.selection+'/Final/'+args.year
    if not os.path.exists(outputDir): os.makedirs(outputDir)
